...
void profile_scan_inclusive(int n, OpenCL& opencl) {
    ...
    opencl.queue.flush();
    cl::Kernel kernel(opencl.program, "scan");
    cl::Kernel kernel_fin(opencl.program, "scan_fin");
    ...
    for(int scan_size = 1; scan_size <= 1024*1024; scan_size *= 1024){
        kernel.setArg(0, d_a);
        kernel.setArg(1, d_a);
        kernel.setArg(2, scan_size);
        if (scan_size == 1024*1024){ NDRange = 10; }
        opencl.queue.enqueueNDRangeKernel(kernel, cl::NullRange, cl::NDRange(n/scan_size), cl::NDRange(NDRange));
    }
...
kernel void reduce(global float* a,
    ...
    if (n < 1024) {
      if (i == 0) {
          group_part[0] = 0;
          for (int k = 0; k < n; k++) { group_part[0] += a[k]; }
        }
    } else {
      group_part[local_i] = a[i];
      barrier(CLK_LOCAL_MEM_FENCE);
      for (int m = get_local_size(0)/2; m > 0; m >>= 1) {
         if (local_i < m) {
            group_part[local_i] += group_part[local_i + m];
         }
         barrier(CLK_LOCAL_MEM_FENCE);
      }
    }
...
kernel void scan(global float* a,
                           global float* result,
                           int step) {
    const int i = get_global_id(0);
    const int local_i = get_local_id(0);
    ...
    local float group_part[1024];
    group_part[local_i] = a[(step-1) + i*step];
    barrier(CLK_LOCAL_MEM_FENCE);

    float sum = group_part[local_i];
    for (int m = 1; m < local_size; m *= 2) {
      if (local_i >= m) {
        sum += group_part[local_i - m];
      }
      barrier(CLK_LOCAL_MEM_FENCE);
      group_part[local_i] = sum;
      barrier(CLK_LOCAL_MEM_FENCE);
    }
    a[(step-1) + i*step] = group_part[local_i];
}

kernel void scan_fin(global float*a, int step) {
        const int i = get_global_id(0);
