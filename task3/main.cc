...
void profile_filter(int n, OpenCL& opencl) {
	...
	float power = pow(1024, i);
	power = power + 0.5 - (power<0);
	int new_power = (int)power;
        scan_kernel.setArg(2, new_power);
        opencl.queue.enqueueNDRangeKernel(scan_kernel, cl::NullRange, cl::NDRange(n/pow(1024, i)), cl::NDRange(1024));
    }
