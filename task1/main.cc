...
kernel void matrix_times_matrix(global float* a,
    ...
    float sum = 0;
    int n_s = n / local_size; 

    int res_i = group_i * local_size + local_i;
    int res_j = group_j * local_size + local_j;

    for(int k = 0; k < n_s; k++){
        local_a[local_j * local_size + local_i] = a[res_j * n + k * local_size + local_i];
        local_b[local_j * local_size + local_i] = b[res_i + local_size * k * n + local_j * n];
        barrier(CLK_LOCAL_MEM_FENCE);
        for(int m = 0; m < n / local_size; ++m){
            sum += local_a[local_j * local_size + m] * local_b[m * local_size + local_i];
            }
        barrier(CLK_LOCAL_MEM_FENCE);
    }
    
    result[res_i + res_j * n] = sum;
    }
